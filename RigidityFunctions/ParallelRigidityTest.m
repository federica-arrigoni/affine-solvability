...
%% Modified by Federica Arrigoni to also include a non-sparse version based on rank
%% If the threshold is not specified then rank is used (suitable for small problems)
%% If the threshold is specified then eigs is performed (suitable for large problems) 
%% The threshold used by Ozyesil was 1e-6 

function rigidityDec = ParallelRigidityTest(AdjMat,d,thresh)
...
if nargin<3
    %disp('rank')
    rigidityDec=size(TestMat,1)==rank(TestMat);
else
    %disp('eigs')
    lambd = eigs(TestMat,1,'sm');
    rigidityDec = lambd>= thresh; % 1e-6 default
end
